...
def inference_model(
    ...
    # 智慧型檢查點偵測：如果有 fold_X 資料夾，自動切換為集成模式
    import glob
    fold_ckpts = glob.glob("models/checkpoints/fold_*/best_model.pt")
    
    if fold_ckpts:
        print(f"偵測到 K-Fold 模型，自動啟動集成模式 (共 {len(fold_ckpts)} 個模型)")
        checkpoint_paths = fold_ckpts
    else:
        checkpoint_paths = [checkpoint_path]
        print(f"使用單一模型模式: {checkpoint_path}")

    print(f"判定門檻設定: Promise={promise_threshold:.2f}, Evidence={evidence_threshold:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--promise-threshold", type=float, default=None, help="承諾判定門檻 (預設自動載入最佳值)")
    parser.add_argument("--evidence-threshold", type=float, default=None, help="證據判定門檻 (預設自動載入最佳值)")
    ...
    if p_thresh is None or e_thresh is None:
        config_path = "outputs/best_thresholds.json"
        if os.path.exists(config_path):
            import json
            with open(config_path, 'r', encoding='utf-8') as f:
                best_params = json.load(f)
                if p_thresh is None: p_thresh = best_params.get('promise_threshold', 0.5)
                if e_thresh is None: e_thresh = best_params.get('evidence_threshold', 0.5)
                print(f"已自動載入最佳門檻設定: {config_path}")
        else:
            if p_thresh is None: p_thresh = 0.5
            if e_thresh is None: e_thresh = 0.5
