...
if __name__ == "__main__":
    ...
    # 測試範例 3: 萬海策略 (G)
    test_text_3 = (
        "萬海將持續落實 ESG 策略，強化董事會監督職能，並秉持誠信經營之核心價值。"
    )
    ...
    for i, t in enumerate([test_text_1, test_text_2, test_text_3]):
        ...
        print(
            f"建議類別: {analysis['suggested_type']} (信心度: {analysis['confidence']})"
        )
        print(f"各項分數: {analysis['scores']}")
        print(
            f"匹配關鍵字: {analysis['detailed_matches'][analysis['suggested_type']]['matches']}"
        )
